...
export const getPosts = (req, res) => {
    ...
    //userInfo is the authenticated users column from the table user
      jwt.verify(token, "secretkey", (err, userInfo) => {
        if(err) return res.status(403).json("Token is not valid")
        
          //SQL query to show only posts where the authenticated user posts and the followed user's posts are showed.
